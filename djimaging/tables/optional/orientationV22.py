...
class OsDsIndexesV22Template(dj.Computed):
    ...
    def make(self, key):
        ...
        if valid:
            ...
            osi_s, pref_or_s = self.get_si(surrogate_v,
                                            sorted_directions,
                                            2)
            null_dist_dsi = self.test_tuning(np.transpose(projected),
                                                sorted_directions,
                                                1)
            p_dsi = np.mean(null_dist_dsi > dsi_s)
            null_dist_osi = self.test_tuning(np.transpose(projected),
                                                sorted_directions,
                                                2)
            p_osi = np.mean(null_dist_osi > osi_s)
            d_qi = self.quality_index_ds(sorted_responses)
            on_off = self.get_on_off_index(u)
    ...
    def gen_null_distribution(self, sorted_response, dirs, per, iters=1000):
        """
        ...
        for i in range(iters):
            ...
            _, v, _ = self.get_time_dir_kernels(np.mean(shuffled_response, axis=-1))
            idx, _, = self.get_si(v, dirs, per)
            null_dist[i] = idx
        return null_dist
    ...
    def test_tuning(self, rep_dir_resps, dirs, per, iters=1000):
        ...
        for i in range(iters):
            ...
            dsi, _ = self.get_si(normalized_shuffled_mean, dirs, per)
            null_dist[i] = dsi

        return null_dist

    def get_on_off_index(self, time_kernel):
        """
