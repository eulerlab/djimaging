...
def test_compute_traces(
        filepath=os.path.join(__RAW_DATA_PATH, "xy-RGCs/20220215/1/Pre/SMP_M1_LR_GCL0_chirp_C1.h5"),
        stack_name='wDataCh0'):
    if not os.path.isfile(filepath):
        pytest.skip(f"File not found {filepath}")
...
def test_compute_tracetimes_line_precision(
        filepath=os.path.join(__RAW_DATA_PATH, "xy-RGCs/20220215/1/Pre/SMP_M1_LR_GCL0_chirp_C1.h5"),
        stack_name='wDataCh0'):
    _test_compute_tracetimes(filepath=filepath, stack_name=stack_name, precision='line', atol=1.5e-3)
